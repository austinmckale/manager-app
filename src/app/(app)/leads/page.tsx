  ...
  return (
    <div className="space-y-4">
      ...
      <section className="space-y-2">
        {leads.map((lead) => (
          <article key={lead.id} className="rounded-2xl border border-slate-200 bg-white p-3">
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="text-base font-semibold text-slate-900">{lead.contactName}</p>
              ...
              <span className="rounded-full bg-slate-100 px-2 py-1 text-[11px] text-slate-700">{lead.stage.replaceAll("_", " ")}</span>
            </div>
        ...
        {leads.length === 0 ? <p className="text-sm text-slate-500">No leads yet.</p> : null}
      </section>
    </div>
  );
}
